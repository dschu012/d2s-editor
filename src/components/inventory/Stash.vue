<template>
  <div class="stash-">
      ...
      <Grid v-if="activeTab == 1" :width="grid.stash.w" :height="grid.stash.h" :page="1"
        :items.sync="stash(0)" @item-selected="onSelect" @item-event="onEvent" :id="'Grid'" :contextMenu=contextMenu></Grid>  
      <Grid v-if="activeTab == 2" :width="grid.stash.w" :height="grid.stash.h" :page="2"
        :items.sync="stash(1)" @item-selected="onSelect" @item-event="onEvent" :id="'Grid'" :contextMenu=contextMenu></Grid>
      <Grid v-if="activeTab == 3" :width="grid.stash.w" :height="grid.stash.h" :page="3"
        :items.sync="stash(2)" @item-selected="onSelect" @item-event="onEvent" :id="'Grid'" :contextMenu=contextMenu></Grid>
      <Grid v-if="activeTab == 4" :width="grid.stash.w" :height="grid.stash.h" :page="4"
        :items.sync="stash(3)" @item-selected="onSelect" @item-event="onEvent" :id="'Grid'" :contextMenu=contextMenu></Grid>
    </div>
  </div>
</template>
